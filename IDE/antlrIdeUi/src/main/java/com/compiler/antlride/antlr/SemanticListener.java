...
public class SemanticListener implements testListener {
	...
	public String GetArithmeticType(ParseTree node)
	{
		...
		case 1:
			...
			else // child(0) is callfunc
			{
				...
				{
					String Content = "cannot Recursively call a function with name << "+CurrentFucName+" >> in qupla";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
		...
		case 2:
			if(node.getChild(0).getText().toLowerCase().equals("not"))
			{
				...
				{
					String Content = "Cannot NOT an operand with type ["+type+"]";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
					ReturnType = "?";
				}
			...
			else if (node.getChild(1).getText().equals("!"))
			{
				...
				{
					String Content = "Cannot Factorial an operand with type ["+type+"]";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
					ReturnType = "?";
				}
		...
		case 5:
			...
			else if (node.getChild(1).getText().equals("?")) // this is for Conditional Expression
			{
				...
				{
					String Content = "cannot Perform Triple Expression with Condition of type ["+firstClause+"]";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
				...
				if ((secondClause.equals("integer") && thirdClause.equals("integer")) ||
					...
					ReturnType = secondClause;
	                    ...
	                    String Content = "MissMatch Types within Triple Expression on 2nd type [" + secondClause + "] and 3rd type [" + thirdClause +"]";
	                    ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
						QuplaCompile.errorlist.add(ei);
	...
	public String GetCompareType(String Left,String Right,String op)
	{
		if(op.equals("AND")||op.equals("OR")||op.equals("XOR"))
		{
			...
			{
				String Content = "Cannot Perform op "+op+" on ["+Left+"] with ["+Right+"]";
				ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
				QuplaCompile.errorlist.add(ei);
				return "?";
			}
				...
				op.equals("GREAT_EQUAL"))
		{
				...
				{
					String Content = "Cannot Compare ["+Left+"] with ["+Right+"]";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
					return "?";
				}
	...
	@Override public void enterDeclarevar(testParser.DeclarevarContext ctx) { 
		try {
			if (ctx.getChildCount() == 3)
			{
				...
				{
					String Content = "Duplicate Variable Declaration with name << "+vm.varName+" >>"+" in Scope of { "+vm.varScope.scopeName+" }";
					ErrorInfo ei = new ErrorInfo("Scope","ic_brace",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
	...
	@Override public void enterDeclarefunc(testParser.DeclarefuncContext ctx) { 
		try {
			if(ctx.getChildCount() == 5 || ctx.getChildCount() == 6)
			{
				...
				{
					String Content = "Duplicate Function Declaration with name << "+sm.scopeName+" >>";
					ErrorInfo ei = new ErrorInfo("Scope","ic_brace",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
				funcNames.add(sm);
	...
	@Override public void enterAssignment(testParser.AssignmentContext ctx) {
		try {
			if(ctx.getChildCount() == 3)
			{
				...
				{
					String Content = "Missing variable declaration with name << "+varName+" >> in Scope of { "+"Global"+" }";
					ErrorInfo ei = new ErrorInfo("Scope","ic_brace",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
				...
				{
					String Content = "Cannot Assign Type ["+arithmeticType+"] into Type ["+AssignLeftType+"]";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
				else if(AssignLeftType.equals("integer") && (arithmeticType.equals("real") || arithmeticType.equals("bool")))
				{
					String Content = "Cannot Assign Type ["+arithmeticType+"] into Type ["+AssignLeftType+"] but can be Cast";
					ErrorInfo ei = new ErrorInfo("Type","ic_question",Content,-1);
					QuplaCompile.errorlist.add(ei);
				}
			...
			if(node.getSymbol().getType() == 36) // 36 is type of token ID
			{
				...
				if(correct)
				{
					...
					if(count == 1)//in this case parent of node is stmt
					{
						...
						{
							String Content = "Missing variable declaration with name << "+varName+" >> in Scope of { "+CurrentScope+" }";
							ErrorInfo ei = new ErrorInfo("Scope","ic_brace",Content,-1);
							QuplaCompile.errorlist.add(ei);
						}
					...
					else//in this case parent of node is except stmt
					{
						...
						{
							String Content = "Missing Function Declaration with name << "+funcName+" >>";
							ErrorInfo ei = new ErrorInfo("Scope","ic_brace",Content,-1);
							QuplaCompile.errorlist.add(ei);
						}
					}
